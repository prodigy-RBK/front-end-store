  ...
  <div class="wrapper">
    ...
    <div id="notifications">
      ...
      <div v-if="quantityNotif" class="alert alertTop alert-danger">
        <div class="container">
          <button type="button" aria-hidden="true" class="close" @click="removeNotify('quantityNotif')">
            <md-icon>clear</md-icon>
          </button>
          <div class="alert-icon">
            <md-icon>info_outline</md-icon>
...
export default {
  ...
  props: {
    image: {
      type: String,
      default: require("@/assets/img/e-commerce/mariya-georgieva.jpg")
    },
    signup: {
      type: String,
  ...
  methods: {
    ...
    addQuantity(index) {
      ...
      this.products[index].productId.availability.forEach(elm => {
        if (elm.color === cartProduct.selectedColor && elm.size === cartProduct.selectedSize) {
      ...
      if (this.deliveryInfo.payment_method === "credit_card") {
        StripeCheckout.configure({
          ...
          token: async function(token) {
            let { data } = await axios.post("https://prodigy-rbk.herokuapp.com/api/stripe/purchase", { token: token.id, amount: orderPrice * 100 });
